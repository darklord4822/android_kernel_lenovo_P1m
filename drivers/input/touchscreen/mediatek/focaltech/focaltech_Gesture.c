...
int fts_Gesture_init(struct input_dev *input_dev)
{
    ...
    input_set_capability(input_dev, EV_KEY, KEY_F10);//nubia add
	...
	__set_bit(KEY_PALM_LOCK, input_dev->keybit);
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
	        ...
	        case GESTURE_DOUBLECLICK:
	                pr_err("GESTURE_DOUBLECLICK\n");
	                input_report_key(input_dev, KEY_F10, 1);//nubia add
	                input_sync(input_dev);
                    input_report_key(input_dev, KEY_F10, 0);//nubia add
	                input_sync(input_dev);
	                break;
	        case GESTURE_O:
...
//    int i = 0;
    ...
    /* FW */
     ...
     {
	 gestrue_id = buf[0];
     if (fts_write_reg(fts_i2c_client, 0xd3, 0)<0)
		printk("Gestruedata Write 0xd3 ok\n");
	else
		printk("Gestruedata Write 0xd3 fail\n");
